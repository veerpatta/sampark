...
```
drizzle/
  schema.ts             the seventeen tables
...
scripts/
  ...
  smoke-ui.ts           every signed-in screen + a marks round + the proxies
...
src/
  app/
    ...
    login/
    api/                requests · r/[token] (+ /photo) · photos · documents
                        search · cron/snapshot · students/import
                        export/{students,marks}.xlsx
  ...
  lib/
    ...
    data-health.ts      completeness by class and field, and the daily snapshot
    activity.ts         what happened lately, across the school
    search.ts           the command palette's one call
...
cannot run at all**, and neither can a document be attached to a child. Nothing
else in the app needs it.

`CRON_SECRET` is what Vercel's cron sends when it calls `/api/cron/snapshot`
...
| The student record: sections, provenance, siblings, timeline, print | **done** |
| Documents on a child — TC, Aadhaar card, mark sheet, as image or PDF | **done** |
| `/students/new` — a child added by hand, through the edit rules | **done** |
| Bulk edit from the board: house, route, section, class, status | **done** |
...
### The fourth door, and what the record now says about itself

`/students/[id]` is five anchored sections — overview, details, documents,
marks, timeline. Under every value is one line saying where it came from:
"PSP import · 12 Mar", "Set by Komal · 3 Sep", "Teacher correction, approved by
Raj · 20 Aug". That is `value_sources` and `change_log` read together
(`src/lib/student-provenance.ts`); the console always knew it and never showed
it. The timeline merges the change log, the teacher's own submissions, marks,
the rounds the child was included in, and documents, with the grouping rules
pure and tested (`src/lib/student-timeline.ts`).

The details are edited a section at a time. Each form posts only its own
fields to the same `saveStudent`; a field absent from the form is left alone,
which is the rule `applyEdits` has always had, so five small forms write
exactly what one big one did. A save may carry a note, and it lands on every
`change_log` row of that save.

**`/students/new` is the fourth door into the master record.** It runs the
ordinary edit rules over a blank record (`planNewStudent`), so a bad phone is
refused there exactly as it is on the child's page; it takes a typed PSP id or
mints a `TMP-` one; and it writes one `change_log` row per field with
`decision = 'created'` and an `office` stamp on each, so the next import cannot
overwrite what the office typed. Gated on `canApproveIntoMaster`, like the
third door.

**A bulk edit is N ordinary edits.** The board's bulk bar sets one group fact
— house, bus route, section, class, status — on a selection, and each child
goes through `editFields` + `applyEdits` on its own: validated, logged and
stamped per child, twenty-five children per batch. A child the rule refuses is
named and skipped; the rest still land. There is no raw `UPDATE … WHERE id IN`.

**Documents are evidence, and removal is a stamp.** `student_documents` holds a
private blob pathname per scan, read only through `/api/documents` with an
admin session. Removing one sets `removed_at`/`removed_by` and deletes the
bytes; the row stays. `app_rw` may UPDATE those two columns and nothing else on
that table — the same column-level shape as `submissions.review_status`.

**The completeness snapshot is stored, not derived.** Yesterday's completeness
cannot be reconstructed from today's rows, so `/api/cron/snapshot` writes one
row per class per field each morning and `/students/health` reads the trend
back. A missed day is filled by the first visit to the dashboard.
