...
**The office's own durable page** (`/t/` on the office row) is the one durable page that may list an Aadhaar, Jan-Aadhaar, date-of-birth or photo round — the only thing it can ever carry is master links, which are the office's own. The exception is a parameter on `isListableOnTeacherPage` keyed on `teachers.is_office`, true for exactly one row, and a test pins that every real teacher is still refused. Revoking it is nulling the column, and the global revoke-all covers it.

**The one link that does reach every group** is the round's **master link** (`audience_kind = 'master'`), and it is the deliberate exception to the paragraph above. It is held by the office rather than by a teacher, and it is a weaker credential than the one its holder already has: whoever receives it has a console login showing the same children behind a password, so it grants no new data — it grants reach without a login. Bounded by: closing the round kills it; `rotateRequestToken` replaces its address in one UPDATE with no window where both work; only `canCreateRequests` can mint or send one and only an owner can rotate it; every send writes a `whatsapp_messages` row naming the number and the sender; and Settings → Office lists every one currently open. See "The master link, and why it is the exception" in the README.
